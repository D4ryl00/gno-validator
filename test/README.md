...
| Var | Default | Meaning |
| --- | --- | --- |
| `GNO_REPO` | `D4ryl00/gno` | gno source repo slug |
| `GNO_VERSION` | pinned commit | branch/tag/commit that carries `tmkms_listener` support |
...
> `GNO_VERSION` is pinned to a commit that has the tmkms feature. Bump it (or set
> the env var) when that support lands on another ref, e.g. upstream master.
